...
from utils.constants import ExperimentState, EVAL_METRICS, IN_DIM_DICT, STEPS_PER_DAY_DICT, NUM_NODES
...
class ExperimentHandler():
    ...
    def loss_function(self, output: Dict[str, Any]) -> torch.Tensor:
        ...
        # e_loss = self.entropy_loss(output['sl'])
        # lp_loss = self.link_pred_loss(output['graph'], output['sl'])
    ...
    def link_pred_loss(self, adj, s_l):
        loss = (adj - s_l.matmul(s_l.transpose(-1, -2))).norm(
            dim=(-1, -2)
        )
        loss = loss / (adj.size(-1) * adj.size(-2))
        return loss.mean()
